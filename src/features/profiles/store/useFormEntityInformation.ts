...
export const useFormEntityInformation = () => {
  ...
  const entityInformationFormRef = useTemplateRef<FormChild>('entityInformationFormChild');

  const isValid = computed(() => entityInformationFormRef.value?.isValid);
  ...
  const handleSave = async () => {
    entityInformationFormRef.value?.onValidate();

    if (!isValid.value) {
      nextTick(() => {
        const element = document.querySelector('.ViewDashboardEntityInformation');
        if (element) {
