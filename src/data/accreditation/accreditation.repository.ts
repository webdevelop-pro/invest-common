...
export const useRepositoryAccreditation = () => {
  ...
  const getAll = async (profileId: number) => {
    try {
      ...
      globalErrorHandling(err, 'Failed to fetch accreditation data');
      throw err;
    } finally {
  ...
  const create = async (profileId: number, note: string) => {
    try {
      ...
      globalErrorHandling(err, 'Failed to create accreditation');
      throw err;
    } finally {
  ...
  const update = async (profileId: number, note: string) => {
    try {
      ...
      globalErrorHandling(err, 'Failed to update accreditation');
      throw err;
    } finally {
  ...
  const uploadDocument = async (userId: number, profileId: number, formData: FormData) => {
    try {
      ...
      globalErrorHandling(err, 'Failed to upload accreditation document');
      throw err;
    } finally {
  ...
  const createEscrow = async (userId: number, profileId: number) => {
    try {
      ...
      globalErrorHandling(err, 'Failed to create escrow');
      throw err;
    } finally {
