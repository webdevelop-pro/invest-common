...
export const fetchGetWalletByProfile = (profile_id: number) => {
  const path = `${EVM_URL}/auth/wallet/${profile_id}`;
...
export const fetchGetTransactionsData = (wallet_id: number) => {
  const path = `${EVM_URL}/auth/wallet/${wallet_id}/transactions`;
...
export const fetchAddBankAccount = (wallet_id: number) => {
  const path = `${EVM_URL}/auth/wallet/${wallet_id}/bank_account`;
...
export const fetchCancelTransaction = (wallet_id: number, transaction_id: number) => {
  const path = `${EVM_URL}/auth/wallet/${wallet_id}/transactions/${transaction_id}`;
...
export const fetchCreateLinkToken = (profileId: number) => {
  const path = `${EVM_URL}/auth/linkaccount/${profileId}/link`;
  const body = JSON.stringify({});
  // ToDo
...
export const fetchCreateLinkExchange = (profileId: number, body: string) => {
  const path = `${EVM_URL}/auth/linkaccount/${profileId}/exchange`;
  const req = {
    body,
    method: 'POST',
    ...requiredFetchParams(),
...
export const fetchCreateLinkProcess = (profileId: number, body: string) => {
  const path = `${EVM_URL}/auth/linkaccount/${profileId}/process`;
  const req = {
    body,
    method: 'POST',
    ...requiredFetchParams(),
...
export const fetchDeleteLinkedAccount = (profileId: number, body: string) => {
  const path = `${EVM_URL}/auth/linkaccount/${profileId}`;
  const req = {
    body,
    method: 'DELETE',
    ...requiredFetchParams(),
